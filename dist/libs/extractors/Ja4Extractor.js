 ...
 */
const JA4_HEADERS = ['x-ja4', 'x-tls-ja4', 'cf-ja4'];
const JA3_HEADERS = ['x-ja3', 'x-tls-ja3', 'cf-ja3-fingerprint'];
const JA3S_HEADERS = ['x-ja3s', 'x-tls-ja3s'];
/**
 ...
 */
const CIPHER_HEADERS = ['x-tls-ciphers', 'x-ssl-ciphers'];
/** TLS extension header — comma-separated decimal type codes */
const EXT_HEADERS = ['x-tls-extensions', 'x-ssl-extensions'];
/** Supported groups (elliptic curves) header */
const CURVES_HEADERS = ['x-tls-groups', 'x-tls-supported-groups'];
 ...
 */
export function extractTlsFromHeaders(headers) {
    const profile = {};
    ...
    if (rawJa3)
        profile.ja3 = rawJa3.trim();
    const rawJa3s = firstHeader(headers, JA3S_HEADERS);
    if (rawJa3s)
        profile.ja3s = rawJa3s.trim();
    const rawCiphers = firstHeader(headers, CIPHER_HEADERS);
    if (rawCiphers)
        profile.cipherSuites = parseCodeList(rawCiphers);
    const rawExts = firstHeader(headers, EXT_HEADERS);
    if (rawExts)
        profile.extensions = parseCodeList(rawExts);
    const rawCurves = firstHeader(headers, CURVES_HEADERS);
    if (rawCurves)
        profile.ellipticCurves = parseCodeList(rawCurves);
    return profile;
}
//# sourceMappingURL=Ja4Extractor.js.map