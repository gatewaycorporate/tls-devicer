 ...
 */
async function loadTlsh() {
    if (_hash !== null)
        return;
    try {
        const mod = await import('tlsh');
        _hash = (mod.default ?? mod);
        const builder = await import('tlsh/lib/digests/digest-hash-builder.js');
        const DigestHashBuilder = (builder.default ?? builder);
        _builder = (hash) => DigestHashBuilder().withHash(hash).build();
    }
    catch {
 ...
 */
export function computeTlshScore(profileA, profileB) {
    if (!_hash || !_builder)
        return null;
    try {
        ...
        if (sa.length < 50 || sb.length < 50)
            return null; // TLSH needs ≥ 50 bytes
        const ha = _hash(sa);
        const hb = _hash(sb);
        if (!ha || !hb)
            return null;
        const distance = _builder(ha).calculateDifference(_builder(hb), true);
 ...
 */
export function computeConsistencyScore(incoming, history, enableJa4 = true, enableJa3 = true, enableHttp2 = true, enableHeaderConsistency = true) {
    const factors = [];
    ...
    if (cipherJaccard < 0.7)
        factors.push('cipher_suite_change');
    // ── Extensions ───────────────────────────────────────────────
    const extensionJaccard = jaccardSimilarity(incoming.extensions, ref.extensions);
    if (extensionJaccard < 0.7)
        factors.push('extension_change');
    // ── HTTP/2 settings ──────────────────────────────────────────
    let http2Score = 1;
    if (enableHttp2) {
        http2Score = http2SettingsSimilarity(incoming.http2Settings ?? {}, ref.http2Settings ?? {});
        if (http2Score < 0.8)
            factors.push('http2_settings_change');
    }
    // ── Header order & values ────────────────────────────────────
    let headerOrderScore = 1;
    let headerValueScore = 1;
    if (enableHeaderConsistency) {
        headerOrderScore = headerOrderSimilarity(incoming.headerOrder ?? [], ref.headerOrder ?? []);
        headerValueScore = headerValueSimilarity(incoming.headerValues ?? {}, ref.headerValues ?? {});
        if (headerOrderScore < 0.7)
            factors.push('header_order_change');
        if (headerValueScore < 0.7)
            factors.push('header_value_change');
    }
    // ── TLSH ─────────────────────────────────────────────────────
    const tlshScore = computeTlshScore(incoming, ref);
    if (tlshScore !== null && tlshScore < 50)
        factors.push('tlsh_distance_high');
    // ── Overall score ─────────────────────────────────────────────
    // Disable-aware weight normalisation
    let totalWeight = 0;
    let weightedSum = 0;
    function add(weight, score, enabled = true) {
        if (!enabled)
            return;
        totalWeight += weight;
 ...
 */
export function computeConfidenceBoost(consistency, weight = 0.15) {
    if (consistency.isNewDevice)
        return 0;
